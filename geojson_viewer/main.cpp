...
class extractor
{
 public:
  ...
  static QVariantMap toVariant(QDeclarativePolygonMapItem* mapPolygon) {
    ...
    if (hasProperties(mapPolygon)) ls["properties"] = mapPolygon->property("props").toMap();
    return ls;
  }
  static QVariantMap toVariant(QDeclarativePolylineMapItem* mapPolyline) {
    ...
    if (hasProperties(mapPolyline)) ls["properties"] = mapPolyline->property("props").toMap();
    return ls;
  }
  ...
  static QVariantMap toVariant(QDeclarativeRectangleMapItem* mapRectangle) {
    ...
    if (hasProperties(mapRectangle)) pt["properties"] = mapRectangle->property("props").toMap();
    return pt;
  }
...
#ifdef Q_OS_ANDROID
  if (!requestStoragePermissions()) return -1;
#endif

  QQmlApplicationEngine engine;
  QUrl absoluteFilePath = argc > 1 ? QUrl(QStringLiteral("file://") + QFileInfo(argv[1]).absoluteFilePath()) : QUrl();
  engine.rootContext()->setContextProperty("dataPath",
                                           QUrl(QStringLiteral("file://") + qPrintable(QT_STRINGIFY(SRC_PATH)) + QStringLiteral("/data")));
  //! [QMLEngine]
